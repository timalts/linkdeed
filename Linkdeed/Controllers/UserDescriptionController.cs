...
namespace Linkdeed.Controllers
{
    ...
    public class UserDescriptionController : ControllerBase
    {
        ...
        [Authorize(Roles = AccessLevel.Admin)]
        [HttpGet("{UserId}")]
        public async Task<ActionResult<UserDescription>> GetDescription_ByUserId(int id)
        {
            var desc = _context.UserDescription.ToList().Find(x => x.UserId == id);
        ...
        [Authorize(Roles = AccessLevel.Admin)]
        [HttpPut("{id}")]
        public async Task<ActionResult> Update_Desc_ByID(int id, UserDescription desc)
        {
            if (id != desc.Id || !DescExists(id))
            {
                return BadRequest();
            }
            else
